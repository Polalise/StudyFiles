...
이 목차는 Markdown 교재와 Jupyter Notebook 실습을 함께 보기 위한 기준 문서입니다.<br>
번호가 `_n`로 붙은 장은 원본 흐름에서 빠지기 쉬운 내용을 보강하기 위해 추가한 연결형 부록 장입니다.
...
4\. [Keras 기본 워크플로우](./04_1_keras_basic_workflow.md)
   ...
   - `Sequential`, `Input`, `Dense`, `compile`, `fit`, `predict`<br><br>
...
7-2. [KMeans 비지도학습 입문](./07_2_kmeans_unsupervised_intro.md)
   ...
   - 정답 `y`가 없을 때 데이터를 묶어보는 기본 비지도학습<br><br>
...
13\. [건강 데이터 프로젝트 시작: 당뇨병 위험 예측 Baseline](./13_health_project_diabetes_baseline.md)
   ...
   - 당뇨병 위험 예측 baseline, 스케일링, confusion matrix<br><br>
...
14\. [임계값, Precision, Recall 이해하기](./14_1_threshold_precision_recall.md)
   ...
   - threshold, precision, recall, F1, FN/FP 해석<br><br>
...
14-2. [Autoencoder 이상 탐지](./14_2_autoencoder_anomaly_detection.md)
   ...
   - reconstruction error와 threshold로 이상 데이터를 판단하는 흐름<br><br>
...
15\. [당뇨병 예측 모델 개선과 실험 관리](./15_experiment_management_diabetes.md)
   ...
   - EarlyStopping, Dropout, 실험 결과표, 최종 모델 선택<br><br>
...
17\. [당뇨병 위험 예측 함수 만들기](./17_diabetes_prediction_function.md)
   ...
   - 모델, scaler, feature order 저장과 예측 함수<br><br>
...
18\. [당뇨병 예측 프로젝트 포트폴리오 문서 만들기](./18_1_diabetes_project_portfolio.md)
   ...
   - README 초안, 한계, 개선 방향, 발표용 요약<br><br>
...
9-2. [Fashion-MNIST CNN 퀴즈](./09_2_fashion_mnist_cnn_quiz.md)
   ...
   - MNIST 다음 단계로 이미지 CNN 분류 복습<br><br>
...
10\. [꽃 이미지 CNN 미니 프로젝트](./10_flower_cnn_mini_project.md)
   ...
   - 폴더 기반 이미지 데이터셋, CNN 학습, 모델 저장<br><br>
...
11\. [꽃 CNN 결과 분석과 오답 확인](./11_flower_cnn_error_analysis.md)
   ...
   - confusion matrix, 클래스별 정확도, 오답 이미지 확인<br><br>
...
12\. [전이학습으로 꽃 분류 개선하기](./12_transfer_learning_flower.md)
   ...
   - MobileNetV2, `include_top=False`, feature extractor<br><br>
...
12-1. [객체 탐지와 YOLO 입문](./12_1_object_detection_yolo.md)
   ...
   - 이미지 분류와 객체 탐지 차이, bounding box, YOLO 개념<br><br>
...
12-2. [YOLO 실제 실행 입문](./12_2_yolo_practical_intro.md)
   ...
   - 사전학습 YOLO 모델 호출, box/class/confidence 읽기<br><br>

12-3. [GAN 입문](./12_3_gan_intro.md)
   - 실습: [12_3_gan_intro.ipynb](./notebooks/12_3_gan_intro.ipynb)
   - 이미지를 분류하는 모델에서 이미지를 생성하는 모델로 확장<br><br>
...
21\. [LSTM으로 PM10 다음 시간 예측하기](./21_1_pm10_lstm_forecasting.md)
   ...
   - LSTM 회귀 모델, MAE/RMSE, `inverse_transform`<br><br>
...
21-2. [RNN, LSTM, GRU, BiLSTM 비교](./21_2_rnn_lstm_gru_bilstm_comparison.md)
   ...
   - 같은 window 데이터로 순환 모델 계열 비교<br><br>
...
22\. [PM10 예측: LSTM과 단순 Baseline 비교하기](./22_pm10_baseline_comparison.md)
   ...
   - naive baseline, mean baseline, LSTM 비교<br><br>
...
23\. [PM10 다변량 LSTM: PM2.5 함께 사용하기](./23_pm10_multivariate_lstm.md)
   ...
   - PM10 + PM2.5 다변량 입력, `(samples, 24, 2)`<br><br>
...
24\. [PM10 시계열 예측 프로젝트 포트폴리오 문서 만들기](./24_pm10_project_portfolio.md)
   ...
   - PM10 프로젝트 README 초안과 발표용 요약<br><br>
...
25\. [경제 시계열 프로젝트 시작: API로 데이터 가져오기](./25_economic_time_series_api_intro.md)
   ...
   - FRED API, `.env`, JSON 응답, 경제 지표 CSV 저장<br><br>
...
26\. [경제 시계열 예측용 데이터셋 만들기](./26_economic_forecasting_dataset.md)
   ...
   - `target_next_cpi`, lag feature, 미래 정보 누출 방지<br><br>
...
27\. [경제 시계열 예측: MLP와 LSTM 비교하기](./27_economic_mlp_lstm_comparison.md)
   ...
   - lag MLP, window LSTM, MAE/RMSE 비교<br><br>
...
28\. [경제 시계열 예측 프로젝트 포트폴리오 문서 만들기](./28_economic_project_portfolio.md)
   ...
   - API 수집, target 설계, 모델 비교, 한계 정리<br><br>
...
29\. [최종 로드맵: 상황에 맞게 딥러닝을 고르는 법](./29_final_roadmap_and_model_selection.md)
   ...
   - 문제 유형별 모델 선택, 대표 포트폴리오 프로젝트 선택, 최종 체크리스트<br><br>

## 추천 학습 순서

1. 처음부터 차례대로 1장부터 7장까지 기본기를 먼저 익힌다.
2. 프로젝트 중심 학습이 필요하면 13장부터 18-2장까지 당뇨병 프로젝트를 완성한다.
